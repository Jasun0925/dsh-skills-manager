  ...
  function createRepositoryUI({ react, Modal, Input, SourceSelect, api, headers, translateError }) {
    ...
    return function useRepositoryUI({ active, t, onInstalled }) {
      ...
      const actions = h(
        ...
        button("repo.refresh", () => perform(async () => {
          for (const repo of repos) await refreshRepo(repo.id);
        }), true, !repos.length),
        button("repo.add", () => {
          setFeedback(null);
      ...
      const groups = visibleRepos.map((repo) => {
        ...
        return h(
          ...
          h("div", { id: panelId, className: "dssm-source-body", hidden: !open }, open ? skills.length ? skills.map((skill) => h(
            ...
            button("repo.detail", () => perform(async () => {
              const detail = await post("detail", { id: repo.id, path: skill.path });
              setModal({ type: "detail", detail, repo });
            })),
      ...
      if (modal) {
        ...
        if (modal.type === "manage") body = h("div", { className: "dssm-repo-panel" }, h("p", { className: "dssm-desc" }, t("repo.removeHint")), feedbackNode, repos.length ? repos.map((repo) => h(
          "div",
          ...
          button("repo.refresh", () => perform(() => refreshRepo(repo.id))),
        ...
        if (modal.type === "detail") body = h("div", { className: "dssm-repo-panel" }, h("div", { className: "dssm-name" }, modal.detail.name), h("p", { className: "dssm-desc" }, modal.detail.description), h("div", { className: "dssm-detail-path" }, modal.repo.owner + "/" + modal.repo.name + " \xB7 " + modal.detail.path), h("div", { className: "dssm-repo-metadata" }, t("repo.commit") + " \xB7 " + modal.detail.commit), h("p", { className: "dssm-note" }, t("repo.installHint")), h("pre", { className: "dssm-code" }, modal.detail.body), feedbackNode);
      ...
      const content = h(
        ...
        !modal ? feedbackNode : null,
        busy ? h("div", { className: "dssm-note", role: "status" }, t("repo.loading")) : null,
