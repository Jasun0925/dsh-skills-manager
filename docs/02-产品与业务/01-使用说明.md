...
更新时间：2026-09-21 07:17（Asia/Shanghai）
...
用户级 Agent 和项目 Agent 来源默认支持外部目录软链接、Windows junction 以及整个技能根链接，无需额外开关。嵌套技能自动发现，目录别名按真实路径去重；循环或失效链接不会阻断扫描，目录过大或过深时页面提示部分技能未显示。可写 DSH 来源保留现有写入边界。

## 技能仓库入口

设置 → 技能 → 技能仓库，可添加公开 GitHub 来源、安装技能、检查更新并回退备份。具体边界和操作见[技能仓库管理](02-技能仓库管理.md)。
...
项目 DSH 与项目 Agent 开关都只更新 manager 本地状态。项目 DSH 按需提供 rank 99 策略覆盖；项目 Agent 始终由 manager 提供 rank 199 候选以加载链接和递归技能，源 `SKILL.md` 始终不变。
