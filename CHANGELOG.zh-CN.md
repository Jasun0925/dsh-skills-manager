...
- 新增 DeepSeek Harness `0.1.7-alpha.1` 兼容支持，保留此前七个版本。设置页图标在宿主提供时使用 0.1.7 的字重导出名，旧宿主继续使用带尺寸后缀的导出名。
- 宿主提供时，技能开关、操作按钮、只读标签、页签和弹窗改用宿主组件。旧宿主缺少这些组件时，用同一套外观本地补上开关、标签和分段页签。打开的开关使用成功色。弹窗里按 Escape 仍只关闭弹窗。
- 旧宿主上的页签保持横排，弹窗打开时焦点落在面板上，而不是右上角关闭按钮。从面板上按 Shift+Tab 仍留在弹窗内。
- 来源和状态收成普通下拉，搜索与它们同一行并靠右。
- 设置页控件改为 Ant Design。按钮、输入框、下拉、页签、开关、标签和弹窗使用它的默认蓝，并跟随宿主的深浅色。原来的宿主控件样式已删除。
...
- 技能详情的正文和 Frontmatter 改为只读多行输入框，字号用默认的 14px，不再使用 11px 灰色等宽字。诊断、正文、Frontmatter 这几个标题也用 14px。
