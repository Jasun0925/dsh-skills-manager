# 更新日志

[English](CHANGELOG.md)

以下记录近期变更；0.1.54 为待发布版本。

## 0.1.54 - 2026-09-21

- 新增公开 GitHub 技能仓库，支持直连归档、校验缓存、安全安装、按文件预览更新、本地修改保护及备份回退。
- 安装来源确认中断后，先核验完整本地目录再恢复记录；保留用户修改，明确提示磁盘写入错误。
- 合并刷新与更新检查，消除重复列表读取及文件哈希计算，仓库下载不再直接占用本地操作队列。
- 优化来源筛选、紧凑技能列表、分页面操作反馈及双语文案；补齐旧宿主颜色变量兜底、仓库大小限制与备份保留说明。
- 公开文档聚焦产品使用，不再纳入内部交接资料及含机器信息的验收证据。

## 0.1.53 - 2026-09-18

- 修复官方 DeepSeek Harness `0.1.6-alpha.2` 上「项目」页为空：会话列表不再提供当前会话字段，项目页改为跟随主界面正在查看的会话；旧宿主仍按原来的当前会话工作。
- 新增 DeepSeek Harness `0.1.6-alpha.2` 兼容支持，保留此前六个版本的支持。
- package.json 描述改为中英双语，与 GitHub 仓库描述一致。

## 0.1.52 - 2026-09-16

- 新增 DeepSeek Harness `0.1.6-alpha.1` 兼容支持，保留此前五个版本的支持；技能发现、启停及管理方式保持不变。
- 规范化隔离宿主沙箱路径，避免 Windows 8.3 临时目录导致 CI 项目技能路径比对失败。

## 0.1.51 - 2026-09-16

- 新增 DeepSeek Harness `0.1.6-alpha.1` 兼容支持，保留此前五个版本的支持；技能发现、启停及管理方式保持不变。

## 0.1.50 - 2026-09-12

- 修复非 Git 工作区的项目技能被跳过的问题。找不到 `.git` 祖先时，使用当前会话 cwd 作为项目根，无需执行 `git init` 即可查看和启停项目技能。

## 0.1.49 - 2026-09-12

- 新增 Copilot 全局 `~/.copilot/skills` 和项目 `.github/skills` 支持，扩展常见 Agent 的项目来源。项目页仅跟随当前会话；直接调用项目管理接口时需携带 `x-dsh-skills-session` 会话 ID。
- 项目副本优先于全局副本，各来源独立启停；停用后可由其他已启用副本接管，并在页面提示实际来源，全部副本停用时才阻断调用。
- 全局、项目和回收站使用独立 Tab，技能按来源折叠并隐藏长描述，精简重复信息。创建和导入固定保存到全局 DSH；通用项目 `skills/` 使用中性来源名称。
- 修复切换中英文时更新按钮和弹窗未同步、回收站时间格式及导入警告分隔符不一致的问题。
