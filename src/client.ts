...
`;
    ...
    function FallbackTabs(props: {label: string; value: string; onChange: (value: string) => void; className?: string; items: {value: string; id: string; panelId: string; label: React.ReactNode}[]}) {
      ...
      return h("div", { className: "dssm-fallback-tabs" + (props.className ? " " + props.className : ""), role: "tablist", "aria-label": props.label, style: { gridTemplateColumns: "repeat(" + count + ", minmax(0, 1fr))" } }, h("span", { className: "dssm-fallback-tabs-indicator", "aria-hidden": true, style: { width: "calc((100% - 8px) / " + count + ")", transform: "translateX(" + (selected * 100) + "%)" } }), props.items.map(function (item, index) {
    ...
    function Modal(props: ModalProps) {
      ...
      function dialogOf() {
        ...
        var dialog = body && body.closest(".dssm-host-modal");
      ...
      react.useLayoutEffect(function () {
        ...
        if (!dialog) return;
        dialog.tabIndex = -1;
        recoverModalFocus(dialog, document.activeElement, document.body);
      });
      react.useEffect(function () {
        ...
        function onKey(event: KeyboardEvent) {
          ...
          var current = dialogOf();
          if (current) trapModalFocus(current, event);
        }
        document.addEventListener("keydown", onKey, true);
        return function () { document.removeEventListener("keydown", onKey, true); };
      }, []);
      return h(primitives.Modal, { open: true, title: props.title, closeLabel: props.closeLabel, onClose: props.onClose, className: "dssm-host-modal" + (props.wide ? " dssm-host-modal-wide" : "") + (props.className ? " " + props.className : "") }, h("div", { ref: bodyRef, className: "dssm-host-modal-body" }, props.children));
    }
