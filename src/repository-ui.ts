...
export const repositoryLocales = {
  zh: {
    ...
    "repo.hint": "从公开 GitHub 仓库或 Bitbucket 仓库发现技能，按需安装到全局 DSH。私有 Bitbucket 使用本机 SSH 密钥。", "repo.footer": "刷新会检查仓库与技能更新，不会自动替换本地文件。历史备份不会自动清理，占用空间会随更新增加。",
    ...
    "repo.empty": "还没有技能仓库，添加一个公开 GitHub 或 Bitbucket 仓库开始。", "repo.noSkills": "没有匹配的技能。可刷新仓库或调整筛选。",
  ...
  en: {
    ...
    "repo.hint": "Discover skills in a public GitHub repository or a Bitbucket repository and install them to global DSH. Private Bitbucket repositories use the SSH key on this computer.", "repo.footer": "Refresh checks repositories and skill updates without replacing local files. Historical backups are not cleaned automatically and use additional disk space.",
    ...
    "repo.empty": "No repositories yet. Add a public GitHub or Bitbucket repository to get started.", "repo.noSkills": "No matching skills. Refresh the repository or adjust your filters.",
  ...
  return function useRepositoryUI({ active, t, onInstalled }: {active: boolean; t: Translate; onInstalled?: () => unknown}) {
    ...
    if (modal) {
      ...
      if (modal.type === "add") body = h("form", { className: "dssm-form", onSubmit: (e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); if (!form.url.trim()) return; perform(async () => { const repo = await post("add", form); await refreshRepo(repo.id); setModal(null); }); } },
        [["url", "repo.url", "https://github.com/owner/repo or https://bitbucket.org/owner/repo"], ["ref", "repo.ref", t("repo.defaultBranch")], ["subdirectory", "repo.directory", t("repo.directoryHint")]].map(([key, label, placeholder]) => h("label", { className: "dssm-field", key }, h("span", { className: "dssm-label" }, t(label)), h(Input, { "aria-label": t(label), value: form[key], placeholder, maxLength: key === "url" ? 2048 : 512, required: key === "url", disabled: busy, onChange: (e: { target: { value: string } }) => setForm({ ...form, [key]: e.target.value }) }))),
